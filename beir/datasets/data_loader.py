...
class GenericDataLoader:
    ...
    def __init__(self, data_folder: str = None, prefix: str = None, corpus_file: str = "corpus.jsonl", query_file: str = "queries.jsonl", 
        ...
        if prefix:
            if query_file == "queries.jsonl": query_file = prefix + "-" + query_file
    ...
    def load(self, split="test") -> Tuple[Dict[str, Dict[str, str]], Dict[str, str], Dict[str, Dict[str, int]]]:
        ...
        if not self.qrels_file: self.qrels_file = os.path.join(self.qrels_folder, split + ".tsv")
        else: self.qrels_file = os.path.join(self.qrels_folder, self.qrels_file + ".tsv")
        self.check(fIn=self.corpus_file, ext="jsonl")
        self.check(fIn=self.query_file, ext="jsonl")
        self.check(fIn=self.qrels_file, ext="tsv")
        ...
        if not len(self.corpus):
            ...
            logger.info("Doc Example: %s", list(self.corpus.values())[0]['text'][:100])
